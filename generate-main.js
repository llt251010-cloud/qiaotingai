...
export default async function handler(req, res) {
  ...
  if (!cleanProduct) {
    return res.status(400).json({ error: "Please enter product name first." });
  }
  ...
  const prompt = [
    "Create one professional Amazon listing product image.",
    `Image type: ${imageType}`,
    `Product: ${cleanProduct}`,
    `Key selling points: ${cleanPoints || "premium quality, clear product details, strong purchase reason"}`,
    `Requested ratio: ${ratio}`,
    "Style: realistic ecommerce product photography, clean commercial lighting, high quality, sharp product details.",
    "Do not include brand logos, watermarks, messy text, unsafe claims, or distorted products.",
  ].join("\n");
  ...
  try {
    ...
    return res.status(500).json({
      error: error?.message || "Server failed to request OpenAI.",
    });
  }
}
